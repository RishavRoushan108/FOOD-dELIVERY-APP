...
export async function POST(request) {
  try {
    ...
    await connectDB();
    const { emailId, password, name, login, phoneNumber } = payload;
    let result;
    let success = false;
    if (login) {
      result = await deliverypatnerModel.findOne({
        emailId,
      });
      ...
      const deliverypatner = new deliverypatnerModel(payload);
      result = await deliverypatner.save();
      if (result) {
        success = true;
      }
    }
